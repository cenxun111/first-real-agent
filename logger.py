...
class AgentLogger:
    ...
    def log_request(self, messages: list[dict], tools: list[Any] | None = None):
        ...
        # Convert simple message dicts to serializable logging format
        for msg in messages:
            msg_dict = {
                "role": msg.get("role", ""),
            ...
            if msg.get("thinking"):
                msg_dict["thinking"] = msg.get("thinking")
            if msg.get("tool_calls"):
                # Deal with litellm objects or raw dicts
                msg_dict["tool_calls"] = []
                for tc in msg.get("tool_calls", []):
                    if hasattr(tc, "model_dump"):
                        msg_dict["tool_calls"].append(tc.model_dump())
        ...
        # Only record tool names from OpenAI format
        if tools:
            request_data["tools"] = [
                tool.get("function", {}).get("name", "unknown") if isinstance(tool, dict) else getattr(tool, "name", "unknown")
                for tool in tools
            ]
    ...
    def log_response(
        ...
        if tool_calls:
            ...
            for tc in tool_calls:
                if hasattr(tc, "model_dump"):
                    response_data["tool_calls"].append(tc.model_dump())
                elif hasattr(tc, "function"):
                    ...
                    response_data["tool_calls"].append(tc)
